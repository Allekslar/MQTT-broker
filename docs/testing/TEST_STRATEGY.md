...
- read-model tests when read-model seams are present or changed
- reducer/effect-flow tests when the execution-model path is present or changed
- async-operation tests when async operation support is present or changed
...
An effective testing strategy for this project is not just a set of tests, but a multi-layer verification system in which:
...
This approach enables the system to grow safely from `single-broker mode` to `federated multi-broker mode` without losing stability.
