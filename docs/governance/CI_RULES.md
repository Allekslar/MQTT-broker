...
- the changed module has no corresponding test or compile coverage where required
- a blocker-level violation has no approved exception in `docs/governance/ADR_EXCEPTIONS.md`
...
During the skeleton phase, the following is acceptable:
- many APIs still return `ERR_UNSUPPORTED_FEATURE`

However, the following is not acceptable:
- unimplemented code appears to succeed
