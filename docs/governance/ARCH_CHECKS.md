...
This document describes the repository's automated architecture checks.
...
- `ARCH-004`: `main/app_main` remains the composition root
- `ARCH-006`: no routing dependency on transport/storage adapters
- `ARCH-007`: test-only access is exposed through separate `*_test_access.hpp` headers
...
- exit with `0` when no mapped script-level violations are present
- exit non-zero when mapped script-level violations are found
- print `rule_id`, file path, and a short remediation hint
- run locally without network dependencies
- gracefully skip code-level checks when the skeleton does not yet exist

---

## 5. PR-level expectations

Required at the PR level:
- grep/static checks
- header layout checks
- forbidden dependency checks
- composition-root checks for `main/app_main`
- test-only access-header layout checks where such seams are required
