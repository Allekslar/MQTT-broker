...
This document defines the practical plan for creating the first compilable code skeleton for the MQTT broker.
...
- compilable structure first, behavior later
...
### 5.3. App-runtime headers

First app-runtime headers:
- `runtime_facade.hpp`
- `read_model_coordinator.hpp`
- `operation_result_store.hpp`

### 5.4. Core public headers
...
Recommended order, aligned with `docs/planning/ROADMAP.md` Stage 1:
1. domain types
2. result/error primitives
3. event primitives
4. port headers
5. `protocol_mqtt` stub
6. `qos` stub
7. `session` stub
8. `retained` stub
9. `ISubscriptionIndex` fake + contract-validation harness
10. `acl` stub
11. `routing` stub
12. `broker_core` stub
13. fake test adapters
14. runtime wiring stub
15. `federation` scaffolding
16. real platform-adapter scaffolding

Reason:
- domain and ports stabilize contracts
- the first core-module stubs follow the Stage 1 roadmap order
