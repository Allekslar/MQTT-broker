...
The key requirement is that the system must grow **without architectural disruption**, while preserving:
...
The goal is not to design "one broker that does everything", but rather:
...
project/
  ...
  components/
    ...
    subscription_index/ -> subscription-index implementation
    qos/                -> /core/qos_engine
    federation/         -> /core/federation_policy
    ports/              -> /ports/*
    transport_tcp/      -> /adapters/tcp_transport
    bridge_link/        -> /adapters/bridge_link
    ...
    app_runtime/        -> runtime facade + read-model application layer
...
The goal is not to design "one broker forever". Instead, design:
- a clean core
- clear interfaces
